...
@section('body')
    <section class="section has-background-black mb-6">
        ...
        <article class="container mt-6s">
            ...
            <div class="" >
                <h2>STATUS: {{$order->delivery_status}}
                    <button class="button is-normal is-clickable has-background-warning" style="border: none;float: right;{{$order->delivery_status === 'completed'? 'display: none;' : ''}}" ><a href="{{route('todo.edit',$order)}}">EDIT</a></button></h2>
            </div>
            <div class=""><h2>PALLET DETAILS: {{$order->pallet_type}}</div><div class="">
...
{{--            <div class=""><h2>DELIVERED BY: {{$order->delivered_by}}</div>--}}
